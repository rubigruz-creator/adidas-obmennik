📄 ОБНОВЛЁННЫЙ ФАЙЛ: Project_map_4.md
markdown
🗺️ КУСОЧНИЦА — Полная техническая карта проекта (v4.1)
📌 ЧАСТЬ 1: СЕРВЕР
Провайдер и доступ
Хостинг: BeGet (VPS)
...
📱 ЧАСТЬ 4: FLUTTER-ПРИЛОЖЕНИЕ (v4.1)
Технологии
...
Структура проекта (обновлённая v4.1)
text
lib/
├── main.dart                 # Точка входа, Material 3, обход SSL, инициализация уведомлений
├── screens/
│   ├── login_screen.dart     # Вход/регистрация, запуск WebSocket
│   ├── files_screen.dart     # Основной экран файлов (ПЕРЕРАБОТАН в v4.1)
│   └── profile_screen.dart   # Экран профиля
├── widgets/
│   ├── file_card.dart        # NEW v4.1: Карточка файла для сетки
│   ├── file_list_tile.dart   # NEW v4.1: Элемент списка файлов
│   ├── folder_card.dart      # NEW v4.1: Карточка папки для сетки
│   ├── folder_list_tile.dart # NEW v4.1: Элемент списка папок
│   ├── file_icon.dart        # NEW v4.1: Иконка по типу файла
│   ├── empty_state.dart      # NEW v4.1: Заглушка для пустой папки
│   ├── breadcrumb_chips.dart # NEW v4.1: Хлебные крошки навигации
...
Зависимости (pubspec.yaml) обновлённые v4.1
yaml
dependencies:
  flutter:
    sdk: flutter
  ...
  intl: ^0.19.0                  # NEW v4.1: Форматирование дат и размеров
Ключевые переменные
dart
...
view_mode — режим отображения (grid/list) NEW v4.1
...
🎨 НОВЫЙ ФУНКЦИОНАЛ v4.1 — ПЕРЕРАБОТКА UI
...
✅ FAB с SpeedDial (длинное нажатие или тап): создать папку, загрузить файл, фото из галереи, фото с камеры
...
FLUTTER ПРИЛОЖЕНИЕ (v4.1):
lib/
│
├── main.dart                 # Material 3, HttpOverrides, инициализация уведомлений
├── screens/
│   ├── login_screen.dart
│   ├── files_screen.dart     # Основной экран (ПЕРЕРАБОТАН v4.1)
│   └── profile_screen.dart
├── widgets/
│   ├── file_card.dart        # NEW v4.1
│   ├── file_list_tile.dart   # NEW v4.1
│   ├── folder_card.dart      # NEW v4.1
│   ├── folder_list_tile.dart # NEW v4.1
│   ├── file_icon.dart        # NEW v4.1
│   ├── empty_state.dart      # NEW v4.1
│   ├── breadcrumb_chips.dart # NEW v4.1
...
✅ Форматирование размера файла (format_file_size.dart)
✅ Относительные даты загрузки (format_date.dart)
✅ Сетка 2 колонки (GridView) с переключением на список
✅ Сохранение режима отображения в SharedPreferences
✅ Хлебные крошки на чипсах (BreadcrumbChips)
✅ Пустая папка с иллюстрацией (EmptyState)
✅ FAB с меню действий (SpeedDial): папка, файл, галерея, камера
✅ Pull-to-refresh (RefreshIndicator)
✅ Фильтры по типу файла под поиском (FilterChip)
✅ Сортировка: по дате, имени, размеру
✅ Сохранение позиции скролла (PageStorageKey)
✅ Улучшенная модаль скачивания с кликабельным путём
✅ SafeArea 90px в BottomSheet меню файла
✅ Чёрный текст на кнопках в меню файла
✅ Кнопка "Скачать" с белым текстом
✅ Обработка ошибок скачивания с кнопкой "Повторить"
✅ Добавлена зависимость intl: ^0.19.0
✅ Обновлён main.dart для Material 3
...
Итого: Система полностью готова. Работает файлообменник с современным Material 3 UI, иерархическими папками, профилями пользователей, прогресс-барами и push-уведомлениями. 🎉