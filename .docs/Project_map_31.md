# 🗺️ КУСОЧНИЦА — Полная техническая карта проекта (v3.1)
...
9. profile.php — управление профилем (НОВЫЙ v3.1)
...
│   └── profile_screen.dart   # Экран профиля (НОВЫЙ v3.1)
├── widgets/
...
НОВЫЙ ФУНКЦИОНАЛ v3.1
📝 Редактирование профиля пользователя
✅ Создан PHP-эндпоинт profile.php (GET/POST)
✅ Валидация и проверка уникальности nickname (3-50 символов)
✅ Обновление полей: nickname, full_name, position
✅ Автоматическое обновление SharedPreferences после смены ника
✅ Экран ProfileScreen с формой редактирования
✅ Иконка профиля (👤) в AppBar рядом с кнопкой выхода

📊 Прогресс-бары при загрузке/скачивании
✅ Визуальный диалог с линейным индикатором при загрузке файлов
✅ Отображение процента выполнения (0-100%)
✅ Поддержка фоновой загрузки без зависания UI
✅ Проверка mounted во всех асинхронных операциях
✅ Отмена таймеров в dispose() для предотвращения ошибок
✅ Для скачивания — диалог с прогрессом (упрощённая версия, стабильная)
...
│   ├── profile.php           # Управление профилем (НОВЫЙ v3.1)
...
└── api_keys (устаревшая, можно удалить)
    ...
    └── created_at
📝 ИСТОРИЯ ИЗМЕНЕНИЙ
v3.1 (текущая) — ПРОФИЛЬ И ПРОГРЕСС-БАРЫ
✅ Добавлен эндпоинт profile.php (GET/POST)
✅ Реализовано редактирование профиля (nickname, full_name, position)
✅ Добавлен экран ProfileScreen в Flutter
✅ Иконка профиля в AppBar
✅ Прогресс-бары при загрузке файлов (диалог + анимация)
✅ Прогресс-бар при скачивании (упрощённая версия)
✅ Исправлены ошибки setState после dispose
✅ Добавлены проверки mounted во все асинхронные методы
...
✅ Переработан FilesScreen: навигация по папкам, хлебные крошки
✅ FAB для создания папок
✅ Длинное нажатие на папку для меню
✅ Пункт "Переместить в папку" в меню файла

v2.0 — УЛУЧШЕНИЯ
✅ Переименование файлов (rename.php + UI)
✅ Выбор "Общий/Личный" при загрузке
✅ Переключение видимости файла из меню
✅ Поиск файлов по имени
...
Итого: Система полностью готова к использованию. Поддержка иерархических папок, профиля пользователя и прогресс-баров работает стабильно. 🎉