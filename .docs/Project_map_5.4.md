# 🗺️ АДИДАС: НАШ ОБМЕННИК — Полная техническая карта проекта (v5.4)
...
# 📱 ЧАСТЬ 4: FLUTTER-ПРИЛОЖЕНИЕ (v5.4 — кросс-платформенное)
...
## Актуальные зависимости (pubspec.yaml v5.4)
...
scp -r "C:\Users\USER\my_api_app\build\web\*" rubi@90.156.171.36:/home/rubi/web/gazonbaza.ru/public_html/web/

# Права на сервере (SSH root)
...
🆕 ЧАСТЬ 6: НОВОЕ В v5.4 — МНОЖЕСТВЕННАЯ ЗАГРУЗКА ФАЙЛОВ
✅ Реализовано
Подэтап 1.1: Множественная загрузка файлов
Выбор нескольких файлов через FilePicker.platform.pickFiles(allowMultiple: true)

Диалог очереди загрузки (MultiUploadDialog) с индивидуальным прогрессом для каждого файла

Общие настройки для всех файлов: видимость (личный/общий) и описание запрашиваются один раз

Кнопка отмены для каждого отдельного файла

Кнопка «Отменить всё» для отмены всех незавершённых загрузок

Автоматическое обновление списка файлов после завершения всех загрузок

Обратная совместимость: одиночная загрузка (pickAnyFile) полностью сохранена

Затронутые файлы:
Файл	Изменения
lib/screens/upload_operations.dart	Добавлены: класс UploadTask, класс MultiUploadDialog, класс MultiUploadDialogState, метод pickMultipleFiles(), метод _startSingleMultiUpload(). Метод _showMultiUploadDialog() вызывает новый диалог.
lib/screens/files_screen.dart	В showFABMenu() заменён вызов pickAnyFile() → pickMultipleFiles()
lib/services/api_service.dart	Используется существующий метод uploadFileBytes() (без изменений)
Архитектура множественной загрузки:
text
showFABMenu() → pickMultipleFiles()
  ├── FilePicker.platform.pickFiles(allowMultiple: true)
  ├── showVisibilityDialog() — один раз для всех
  ├── showDescriptionDialog() — один раз для всех
  ├── Создание List<UploadTask>
  └── _showMultiUploadDialog(tasks)
        └── MultiUploadDialog (StatefulWidget)
              ├── initState() → widget.onStart(task) для каждого файла
              ├── _startProgressUpdater() — обновление UI каждые 100ms
              └── Для каждого файла:
                    └── ApiService.uploadFileBytes(token, bytes, fileName, ...)
                          ├── onSuccess → task.isCompleted = true
                          └── onError → task.hasError = true
⚠️ ОШИБКИ, ВСТРЕТИВШИЕСЯ ПРИ РАЗРАБОТКЕ, И ИХ РЕШЕНИЯ
Ошибка 1: Tree-shaking вырезал приватный класс _MultiUploadDialog
Симптом: После сборки в JS-файле отсутствовал код _startSingleMultiUpload и uploadFileBytes. Файлы выбирались, но загрузка не происходила.

Причина: Dart-компилятор вырезает приватные классы (начинающиеся с _), если не видит их использования извне файла. Класс _MultiUploadDialog был приватным и использовался только внутри миксина UploadOperations.

Решение: Переименовать _UploadTask → UploadTask, _MultiUploadDialog → MultiUploadDialog, _MultiUploadDialogState → MultiUploadDialogState. Публичные классы гарантированно попадают в скомпилированный код.

Проверка: После исправления в консоли браузера появляются логи _startSingleMultiUpload: START и _startSingleMultiUpload: DONE, файлы успешно загружаются.

Ошибка 2: PowerShell съедал символ $ в heredoc-строках
Симптом: При попытке записать код через @'...'@ в PowerShell, строки типа $fileName и $e интерпретировались как переменные PowerShell.

Решение: Использовать VSCode для замены содержимого файлов напрямую, либо использовать python-скрипт для записи файла.

Ошибка 3: uploadFileBytesWithProgress не отправлял данные на сервер
Симптом: Прогресс доходил до 90% (имитация через таймер), но реальная загрузка не происходила.

Причина: Метод uploadFileBytesWithProgress в api_service.dart использует http.StreamedRequest, но не отправляет тело запроса.

Решение: Использовать проверенный метод uploadFileBytes (без имитации прогресса) для множественной загрузки. Прогресс показывается на уровне UI через счётчик завершённых задач.

Статус: uploadFileBytesWithProgress требуется доработка в будущем, если понадобится реальный прогресс для отдельных файлов.

Ошибка 4: Локальный запуск через localhost вызывал CORS-ошибки
Симптом: Access to fetch at 'https://gazonbaza.ru/login.php' from origin 'http://localhost:8080' has been blocked by CORS policy

Решение: Тестировать множественную загрузку напрямую на production-домене https://gazonbaza.ru/app (находится на том же домене, CORS не требуется). Сборка и деплой занимают ~2 минуты.

🆕 ЧАСТЬ 7: ИСПРАВЛЕНИЯ В v5.3 (СОХРАНЕНЫ)
...
feature/web-version — веб-версия (v5.2–v5.4)

Документация: docs/ (карта проекта, инструкция, презентация)

📜 ИСТОРИЯ ВЕРСИЙ
v5.4 (текущая) — МНОЖЕСТВЕННАЯ ЗАГРУЗКА
...
v1.0 — БАЗОВАЯ ВЕРСИЯ
🔜 ПЛАНЫ НА БУДУЩЕЕ
Задача 1.2: Множественная обработка файлов (в процессе)
Режим выбора (долгое нажатие, чекбоксы на карточках)

AppBar с количеством выбранных и кнопками действий
...
Таблица audit_log в MariaDB (id, user_id, action, file_id, folder_id, details JSON, created_at)
...
🚀 Статус v5.4: Множественная загрузка файлов реализована и работает. Проект готов к следующему этапу — множественная обработка (удаление/перемещение) и аудит действий.
